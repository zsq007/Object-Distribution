...
int controller::run_20_random(int _move_para)
{
	int i, j, k;
	agent::message msg;
	vector<vector <int> > same_pos_agt;
	...
	while(!check_terminate())
	{
		...
		for(i = 0; i < agent_num; i++)
		{
			...
			{
				objnum.clear();							// clear the vector obj_num
				objnum = agt[i].decision_20_random(same_pos_agt[i][j]);						// Run the distributed algorithm
				if(objnum.empty())
					continue;
				for(k = 0; k < objnum.size(); k++)
				{
...
int controller::run_20_interest(int _move_para)
{
	int i, j, k;
	agent::message msg;
	vector<vector <int> > same_pos_agt;
	...
	while(!check_terminate())
	{
		...
		for(i = 0; i < agent_num; i++)
		{
			...
			{
				objnum.clear();							// clear the vector obj_num
				objnum = agt[i].decision_20_interest(same_pos_agt[i][j]);						// Run the distributed algorithm
				if(objnum.empty())
					continue;
				for(k = 0; k < objnum.size(); k++)
				{
