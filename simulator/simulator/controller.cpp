...
controller::controller(int map_size, int agent_number, int max_round, pair<int,int> distance_range, pair<int,int> turning_probability, pair<int,int> topic_object)
{
	...
	{
		for (int j = 0; j < topic_object.second; j++)
		{
			temp_obj.type = i;
			temp_obj.num = j;
	...
	for(int i = 0; i < agent_num; i++)
	{
		...
		for (int j = 0; j < topic_object.first; j++)
		{
			I[i][j].resize(topic_object.second);
			for (int k = 0; k < topic_object.second; k++)
				I[i][j][k] = false;
		}
	}
	for(size_t j = 0; j < mem_pool.size(); j++)
	{
		for(size_t k = 0; k < mem_pool[j].size(); k++)
		{
