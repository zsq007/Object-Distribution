	...
	while(!check_terminate())
	{
		...
		for(i = 0; i < agent_num; i++)
		{
			for(j = 0; j < same_pos_agt[i].size(); j++)
			{
				...
				objnum = agt[i].decision_naive(same_pos_agt[i][j]);						// Run the distributed algorithm
				if(objnum.empty())
					continue;
...
int controller::run_k_random(int _move_para, int obj_limit)
{
	int i, j, k;
	agent::message msg;
	vector<vector <int> > same_pos_agt;
	...
	while(!check_terminate())
	{
		...
		for(i = 0; i < agent_num; i++)
		{
			...
			{
				objnum.clear();							// clear the vector obj_num
				objnum = agt[i].decision_k_random(same_pos_agt[i][j], obj_limit);						// Run the distributed algorithm
				if(objnum.empty())
					continue;
				for(k = 0; k < objnum.size(); k++)
				{
...
int controller::run_k_interest(int _move_para, int obj_limit)
{
	int i, j, k;
	agent::message msg;
	vector<vector <int> > same_pos_agt;
	...
	while(!check_terminate())
	{
		...
		for(i = 0; i < agent_num; i++)
		{
			...
			{
				objnum.clear();							// clear the vector obj_num
				objnum = agt[i].decision_k_interest(same_pos_agt[i][j], obj_limit);						// Run the distributed algorithm
				if(objnum.empty())
					continue;
				for(k = 0; k < objnum.size(); k++)
				{
