...
vector<pair <int,int> > agent::decision_k_random(int to_id, int obj_limit)
{
	int j, k;
	vector<pair <int, int> > object_coordinate;
	// Random method: send at most obj_limit random objects
...
vector<pair <int,int> > agent::decision_k_interest(int to_id, int obj_limit)
{
	int i, j, k;
	int temp_rand, round_left;
	vector<pair <int, int> > temp_object_coordinate, object_coordinate;

	// obj_limit objects send to each agent, judging by the Interest table
	for (j = 0; j < public_mem_map[to_id].size(); j++)
	{
		for (k = 0; k < public_mem_map[to_id][j].size(); k++)
		{
	...
	if (temp_object_coordinate.size() < obj_limit)
	...
	for (i = 0; i < temp_object_coordinate.size(); i++)
	{
		if (I[to_id][temp_object_coordinate[i].first][temp_object_coordinate[i].second])
		{
			...
			if (object_coordinate.size() >= obj_limit)
				return object_coordinate;
	...
	round_left = obj_limit - object_coordinate.size();
